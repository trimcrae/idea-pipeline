...
FEEDS = [
    # ------------------------------------------------------------------ trucking
    {
        ...
        "faq": [
            ("Is this the same as a 'new MC authority' list?", "It is earlier: every new USDOT registration (for-hire carriers, private fleets, brokers) in the week it was added to the FMCSA census. Authority grants follow days to weeks later — see the separate MC-authority feed."),
    ...
    # ------------------------------------------------------- permits & rentals
    {
        ...
        "short": "Every initial (not renewal) DOB permit issued in the past 7 days — job type, work type, address, borough, owner business, contractor business and phone — weekly CSV.",
        ...
        "price": 19,
        "source": {"domain": "data.cityofnewyork.us", "dataset": "ipu4-2q9a", "date_field": "issuance_date", "date_format": "mixed",
                   "where": "filing_status = 'INITIAL'", "order": "job__ DESC", "id_field": "permit_key"},
        "columns": [("job__", "job"), ("permit_sequence__", "permit_seq"), ("issuance_date", "issued"), ("job_type", "job_type"),
                    ("permit_type", "permit_type"), ("work_type", "work_type"), ("house__", "house"), ("street_name", "street"),
                    ("borough", "borough"), ("zip_code", "zip"), ("bldg_type", "building_type"), ("residential", "residential"),
                    ("owner_s_business_name", "owner_business"), ("owner_s_business_type", "owner_type"),
                    ("permittee_s_business_name", "contractor_business"), ("permittee_s_license_type", "contractor_license_type"),
                    ("permittee_s_phone__", "contractor_phone")],
        "derive": lambda raw, rec: rec.update(issued=_mdy(rec["issued"]) if "/" in rec["issued"] else rec["issued"][:10],
                                              permit_key=rec["job"] + "-" + rec["permit_seq"]),
        "extra_key": "permit_key",
        "contact": {"contractor_phone": "phone"},
        "group_by": "borough",
        "attribution": "NYC Department of Buildings — Permit Issuance (NYC Open Data, updated daily)",
...
CATEGORIES = [  # hub order + labels
    ("trucking", "Trucking & freight", "FMCSA registrations, authority grants and revocations, insurance cancellations — nationwide."),
